...
def navbar (request):
    ...
    return render (request,'_navbar.html',{'user_detail':user_detail})
...
# def employer_profile_page (request,username):
#     # user = get_object_or_404(User, username=username)
#     employer_details = EmployerDetails.objects.filter( user__username=username).first()

#     # return HttpResponse (employer_details)
#     return render (request,'Candidate/employer_profile_page.html',{'employer_details':employer_details})
...
def employer_login(request):
    if request.method == 'GET':
        ...
        if username and employer_password:
            employer = authenticate(request, username=username, password=employer_password)
            if employer is not None and employer.is_staff:
                if  employer.is_staff:
                    login(request, employer)
                    messages.success(request, 'Logged In successfully!')
                    return redirect('home')  # Assuming 'home' is the name of your home URL
                else:
                    messages.error(request, "User is not authorized to login as employer.")
            else:
                messages.error(request, "Username or Password is incorrect")
        else:
            messages.error(request, "Username and Password are required.")
    ...
    # return HttpResponse(user)
        if request.method == 'GET':
            ...
            if form.is_valid :
                ...
                form.save()
                messages.success(request,'Record updated Successfully!')
                return redirect(user_details_edit,username=user.username)
            else:
                # return HttpResponse (form.errors)
                messages.error(request,'invalid data')
...
def user_details_edit(request, username):
    if request.user.is_active:
     ...
     if request.method == 'GET':
        ...
        full_user_details_form = FullUserDetailsForm(request.POST, instance=user_details)
        ...
        if full_user_details_form.is_valid() and specialization:
            ...
            return redirect('user_details_edit',username=username)
